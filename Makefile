...
KIND_CLUSTER ?= vworkspace-operator-test-e2e
...
MOCK_CONTROL_PLANE_IMAGE ?= example.com/mock-control-plane:v0.0.1
MOCK_ODOO_IMAGE ?= $(MOCK_CONTROL_PLANE_IMAGE)
.PHONY: docker-build-mockcontrolplane docker-build-mockodoo
docker-build-mockcontrolplane: ## Build docker image with the in-cluster mock control plane server.
	$(CONTAINER_TOOL) build -f Dockerfile.mockcontrolplane -t ${MOCK_CONTROL_PLANE_IMAGE} .

docker-build-mockodoo: docker-build-mockcontrolplane ## Deprecated alias for docker-build-mockcontrolplane.
...
docker-buildx: ## Build and push docker image for the manager for cross-platform support
	...
	- $(CONTAINER_TOOL) buildx create --name vworkspace-operator-builder
	$(CONTAINER_TOOL) buildx use vworkspace-operator-builder
	- $(CONTAINER_TOOL) buildx build --push --platform=$(PLATFORMS) --tag ${IMG} -f Dockerfile.cross .
	- $(CONTAINER_TOOL) buildx rm vworkspace-operator-builder
	rm Dockerfile.cross
...
# KUBECTL_WAIT=false skips waiting for resources to vanish (e2e teardown; kind cluster is deleted next).
KUBECTL_WAIT ?= true
KUBECTL_WAIT_FLAG := $(if $(filter false,$(KUBECTL_WAIT)),--wait=false,)

.PHONY: uninstall
uninstall: manifests kustomize ## Uninstall CRDs from the K8s cluster specified in ~/.kube/config. Call with ignore-not-found=true to ignore resource not found errors during deletion.
	@out="$$( "$(KUSTOMIZE)" build config/crd 2>/dev/null || true )"; \
	if [ -n "$$out" ]; then echo "$$out" | "$(KUBECTL)" delete --ignore-not-found=$(ignore-not-found) $(KUBECTL_WAIT_FLAG) -f -; else echo "No CRDs to delete; skipping."; fi
...
undeploy: kustomize ## Undeploy controller from the K8s cluster specified in ~/.kube/config. Call with ignore-not-found=true to ignore resource not found errors during deletion.
	"$(KUSTOMIZE)" build config/default | "$(KUBECTL)" delete --ignore-not-found=$(ignore-not-found) $(KUBECTL_WAIT_FLAG) -f -
