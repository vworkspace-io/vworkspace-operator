...
3. **A Helm chart.** Packaged as a `.tgz` and published to **GitHub Releases** on every `v*` tag (along with `crds.yaml`, `operator.yaml`, and `SHA256SUMS`). The chart's `appVersion` matches operator SemVer; set `image.tag=vX.Y.Z` at install time to match the Docker Hub image tag. OCI chart registry (`ghcr.io`) remains on the roadmap — see [../install/helm.md](../install/helm.md#install-from-github-release).
