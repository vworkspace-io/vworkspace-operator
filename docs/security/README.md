...
4. [authentication.md](authentication.md) — Cluster identity, one-time registration tokens, long-lived bootstrap credentials, scoped tokens per cluster, optional mTLS, optional signed/encrypted payloads, token rotation, and how Odoo authenticates operator status posts.
...
The full threat model is in [threat-model.md](threat-model.md). The condensed version, in plain prose: the operator holds an outbound credential to Odoo (Pull mode) or accepts inbound writes from Odoo (Push mode) or syncs from Git (GitOps mode). Everything it does on the cluster — applying CRDs, creating `HelmRelease`s, creating `velero.io/Backup`s, creating `Workflow`s, creating `Job`s — is constrained to namespaces labeled `app.vworkspace.io/managed-by=vworkspace`. The Helm engine, Velero, external-secrets, and the workflow runner run under their own service accounts that the operator does not impersonate. Cluster-scoped power is limited to the operator's own CRDs.
...
- Emits a Kubernetes `Event` on the affected resource and a coalesced audit event to Odoo's `POST /api/agent/events` endpoint (Pull mode) or Odoo's equivalent in Push/GitOps mode.
